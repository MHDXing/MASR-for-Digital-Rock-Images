        ...
        for i in range(self.n_HG):
            ...
            if i in super_block_idx:
                ...
                if super_block_idx.index(i) == 0:
                    ...
                    if self.out_nc == 3:
                        flat_r, flat_g, flat_b = flat_map.split(split_size=1, dim=1)  # 张量分割，单个分块的形状大小，分块维度
                        edge_r, edge_g, edge_b = edge_map.split(split_size=1, dim=1)
                        corner_r, corner_g, corner_b = corner_map.split(split_size=1, dim=1)  # 将个通道都分割出来
                        r_map = torch.cat((flat_r, edge_r, corner_r), dim=1)  # 按通道连接flat、edge、corner
                        g_map = torch.cat((flat_g, edge_g, corner_g), dim=1)
                        b_map = torch.cat((flat_b, edge_b, corner_b), dim=1)
                        r_map = F.softmax(r_map, dim=1)  # 按通道激活
                        g_map = F.softmax(g_map, dim=1)
                        b_map = F.softmax(b_map, dim=1)
                        flat_r, edge_r, corner_r = r_map.split(split_size=1, dim=1)  # 将每个成分分割出来
                        flat_g, edge_g, corner_g = g_map.split(split_size=1, dim=1)
                        flat_b, edge_b, corner_b = b_map.split(split_size=1, dim=1)
                        flat_map = torch.cat((flat_r, flat_g, flat_b), dim=1)  # 将每个成分的三通道组合起来
                        edge_map = torch.cat((edge_r, edge_g, edge_b), dim=1)
                        corner_map = torch.cat((corner_r, corner_g, corner_b), dim=1)
                    else:
                        map = torch.cat((flat_map, edge_map, corner_map), dim=1)
                        map = F.softmax(map, dim=1)
                        flat_map, edge_map, corner_map = map.split(split_size=1, dim=1)
...
#         #Actually, the ksize can be any size for all scales
#         self.upsample = make_upsample_block(upscale=upscale)
...
#         srout = self.upsample(sr_feature)
